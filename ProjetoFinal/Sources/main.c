/*
 *
 * Este programa mede a dist�ncia entre o sensor HC-SR04 e um objeto de interesse.
 * A dist�ncia � exibida em centimetros no display LCD.
 * As distancias captadas pelo sensor variam de 3 a 100 cm, aproximadamente.
 * Eh possivel inserir um valor desejado de distancia no terminal do CW apertando o botao S1 da placa shield.
 * Assim que o valor inserido for alcancado uma mensagem sera exibida no terminal.
 *
 */
...
int main(void) {
	...
	char term_cursor_backward[6] = {ESC,'[','3','8','D'};
	char read[4];
	char insert_msg[] = "Insira distancia desejada (em cm): ";
    ...
    for(;;) {
    	...
    	if (input_flag == 1) {
			...
			puts_UART0(term_cursor_down);
			puts_UART0(term_cursor_backward);
			puts_UART0("Pressione ENTER para continuar");
			while (c != ENTER) {
			}
			c = '\0';
			puts_UART0(term_clear);
			puts_UART0(term_pos_reset);
			puts_UART0("Varie a distancia do objeto ao sensor");
			input_flag = 0;
			i = 0;
    	}
    	...
    	if (flag == 1) {
			...
			aux[4] = 'm';
			aux[5] = '\0';
			if (aux[0] == read[0] && aux[1] == read[1] && aux[2] == read[2]) {
				puts_UART0(term_clear);
				puts_UART0(term_pos_reset);
				puts_UART0("Distancia alcancada");
				read[0] = 0;
				read[1] = 0;
				read[2] = 0;
			}
